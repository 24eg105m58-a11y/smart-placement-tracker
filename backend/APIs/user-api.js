...
userApp.post("/register", upload.single("profileImageUrl"), async (req, res, next) => {
  ...
  try {
    ...
    if (!newUser.password || typeof newUser.password !== "string" || newUser.password.length < 8) {
      return res.status(400).json({
        success: false,
...
userApp.post("/login", async (req, res, next) => {
  try {
    ...
    const user = await UserModel.findOne({ email, isUserActive: true });
...
// soft delete user
userApp.put("/soft-delete/:id", verifyToken("ADMIN"), async (req, res, next) => {
  try {
